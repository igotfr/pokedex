...
use rusqlite::{Connection, OpenFlags, Error::SqliteFailure, params_from_iter, params};
...
impl Repository for InMemoryRepository {
  fn insert(&self, number: PokemonNumber, name: PokemonName, types: PokemonTypes) -> Result<Pokemon, InsertError> {
    ...
    if lock.iter().any(|pokemon| pokemon.number == number || pokemon.name == name) {
...
impl AirtableRepository {
    pub fn try_new(api_key: &str, workspace_id: &str) -> Result<Self, ()> {
      let url = format!("https://api.airtable.com/v0/{workspace_id}/pokemons");
    ...
    fn fetch_pokemon_rows(&self, number: Option<u16>) -> Result<AirtableJson, ()> {
      let url = match number {
        Some(number) => format!("{}?filterByFormula=number%3D{number}", self.url),
...
impl SqliteRepository {
  pub fn try_new(path: &str) -> Result<Self, ()>{
    ...
    match connection.execute("pragma foreign_kes = 1", []) {
      Ok(_) => Ok(Self { connection: Mutex::new(connection) }),
      _ => Err(()),
    }
  }

  fn fetch_pokemon_rows(lock: &MutexGuard<'_, Connection>, number: Option<u16>) -> Result<Vec<(u16, String)>, ()> {
